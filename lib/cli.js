...
cli.responders.help = () => {
  ...
  const commands = {
    ...
    'more log info --{fileName}': 'show specific log info',
  };
...
// more log info
// 4kwjygogaaxgplw64e5o-1636265361855
cli.responders.moreLogInfo = (str) => {
	const fileName = str.split('--')[1];

	if (!fileName) return console.log('Missing required parameter --{fileName}');

	_logs.decompress(fileName, (err, logStr) => {
		if (err) console.error(err);

		cli.verticalSpace(1);

		if (!logStr) return console.log('log is empty')
